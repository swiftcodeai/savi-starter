...
const ChildrenTickingHotspots = () => {
  ...
  const handleSmallImageClick = (id: number) => {
    ...
    // Play the vocab audio
    const vocab = VOCAB_ITEMS.find(v => v.id === id);
    if (vocab) {
      const audio = new Audio(`/audio/words/${vocab.word.toLowerCase()}.webm`);
  ...
  // Also play audio when clicking on the completed items in the right panel
  const handleCompletedItemClick = (word: string) => {
        ...
        <div className="flex flex-col w-64 overflow-y-auto max-h-[480px] bg-white rounded-xl border-2 border-gray-300 shadow-lg p-4 space-y-3 select-none">
          {VOCAB_ITEMS.map(({ id, word, image }) => (
            <div
              ...
              className="flex items-center gap-3 p-2 rounded-lg border border-gray-200 bg-gray-50 hover:bg-savi-blue/20 transition cursor-pointer"
              onClick={() => tickeds.has(id) && handleCompletedItemClick(word)}
            >
              {tickeds.has(id) && (
                <img
                  src={image}
...
const Page: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-sky-100 flex flex-col">
      ...
      <LessonLayout
        className="max-w-6xl mx-auto"
        nextPath={LessonSequence["2.2"]}
        prevPath={LessonSequence["1.2.2"]}
      >
        <ChildrenTickingHotspots />
      </LessonLayout>
    </div>
  );
};
